...
meal_gap_plot <- ggplot(d, aes(fill = source, y = pct_covered, x = date)) +
  ...
  labs(x = "", y = "Percent Covered", fill = "Meal Source") +
...
#| fig-width: 10
#| fig-height: 7
#| fig-cap: Bars appearing in the green section, with values less than zero, indicate a meal surplus for that month
...
gap_over_time <- ggplot(d_2, aes(x = date, y = meal_gap, fill = neighborhood)) +
  ...
  theme_minimal() +
  ggsci::scale_fill_jama() +
  labs(x = "", y = "Meal Gap (% Meals Short)", fill = "Neighborhood") +
  scale_x_date(date_breaks = "6 months", date_labels = "%b %Y") +
